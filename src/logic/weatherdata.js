...
export async function getCurrentWeatherData(search) {
  const currentResponse = await fetch(
    `https://api.weatherapi.com/v1/current.json?key=e27d0611824649cba3880002231612&q=${search}&aqi=no`,
    { mode: "cors" },
  );
...
export async function getForecastWeatherData(search) {
  const forecastResponse = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=e27d0611824649cba3880002231612&q=${search}&days=3&aqi=no&alerts=no`,
    { mode: "cors" },
  );
